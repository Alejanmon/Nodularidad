...
def main():
    ...
    with open(output_file, mode='w') as f1:
        ...
        print("Tamaño mínimo de grafito, {:.3f}".format(min_grainsize), file = f1)
        # print("丸み係数のしきい値, {:.3f}".format(marumi_ratio), file = f1)
        print("Umbral del factor de redondez, {:.3f}".format(marumi_ratio), file = f1)
        # print("画像処理と出力画像の幅, {}".format(pic_width), file = f1)
        print("Procesamiento de imágenes y ancho de la imagen de salida, {}".format(pic_width), file = f1)
        # print("ファイル名, 球状化率_ISO法(%), 球状化率_JIS法(%)", file = f1)
        print("Nombre de archivo, Tasa de esferoidización_método ISO (%), Tasa de esferoidización_método JIS (%)", file = f1)
